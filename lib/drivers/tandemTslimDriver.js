  ...
  */              var recordType = getLogRecordById(rec.header_id);
        ...
        tandemCommandResponse(COMMANDS.LOG_SIZE_REQ, null, function (err, result) {
            ...
            if (err) {
                ...
                if (result.valid && (result.descriptor == RESPONSES.LOG_SIZE_TE.value)) {
                    ...
                    start_seq = result.payload['start_seq'];
    ...
    var buildSettingsRecords = function buildSettingsRecord(data, postrecords) {
        ...
        var postsettings = cfg.builder.makeSettings()
            ...
            .with_time(data.profiles[0].timestamp)
            .with_deviceTime(data.profiles[0].displayTime)
            .with_timezoneOffset(data.profiles[0].timezoneOffset)
            .done();
    ...
    var buildBolusRecords = function (data, records) {
        ...
        for (var key in boluses) {
            ...
            if ((bolus.type == 'standard' || bolus.type == 'quickbolus') && bolus.bolus_size === undefined) { // cancelled before any insulin was given
                record.with_time(bolus.timestamp)
                    .with_deviceTime(bolus.deviceTime)
                    .with_normal(0);
            ...
            if (bolus.total_bolus_size !== undefined && bolus.target_bg !== 0 && bolus.target_bg) {
                var wizard_record = cfg.builder.makeWizard()
                    ...
                    .with_bgTarget({low: bolus.target_bg, high: bolus.target_bg, target: bolus.target_bg})
                    .with_bolus(record)
                    .with_units('mg/dL')
                    //.with_payload()
                    .done();
    ...
    var buildBasalRecords = function (data, records) {
        ...
        var finalizeBasal = function(event, basal, currentTemp) {
            ...
            if (duration != basal.duration) {
                //basal.with_expectedDuration(currentTemp.duration);
                if (currentTemp) {
                    basal.expectedDuration = currentTemp.expectedDuration;
    ...
    var buildCartridgeChangeRecords = function (data, records) {
        var cartridgeRecords = filterLogEntries([PUMP_LOG_RECORDS.LID_CARTRIDGE_FILLED],
            data.log_records);
    ...
    return {
        ...
        uploadData: function (progress, data, cb) {
            ...
            var sessionInfo = {
                ...
                // TODO: replace with cfg.tzoUtil.type
                timeProcessing: 'foo',
                tzName: cfg.timezone,
                version: cfg.version
            };
