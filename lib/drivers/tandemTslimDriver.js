...
module.exports = function (config) {
  ...
  var tandemFetch = function (progress, data, callback) {
    ...
    var headerIdFilter = [13,36,37,67,50];
    ...
    function getNewestEvent(err, result) {
      ...
      if (err) {
        ...
        if (headerIdFilter.indexOf(result.payload.header_id) > -1 || result.payload.name === undefined) {
          console.log('filtered record: ', result.payload.header_id + ', ' + result.payload.name);
          ...
          console.log('newest record: ', result.payload.jsDate);
    ...
    function getOldestEvent(err, result) {
      ...
      if (err) {
        ...
        console.log('oldest record: ', result.payload.jsDate);
        console.log('oldest: ', result);
        minIndex = start_seq;
    ...
    function binarySearch(err, result) {
      ...
      if (err) {
        ...
        if (minIndex <= maxIndex) {
          ...
          console.log('record time: ', result.payload.jsDate);
          console.log('target: ', searchElement);
          console.log('record: ', result.payload.rawTimestamp);
          ...
          console.log('90 day, closest record: ', result.payload.jsDate);
      ...
      switch (event.change_type) {
        ...
        case 'pump_resumed':
          ...
          postbasal = cfg.builder.makeScheduledBasal()
            .with_scheduleName(_.find(data.profiles,{idp:event.idp}).name)
